...
import { Users, Calendar, Target, Search, MapPin, ArrowRight, Loader, Clock, Activity } from 'lucide-react';
  ...
  useEffect(() => {
    const fetchConnections = async () => {
      try {
        ...
        setConnections(all.filter(c => String(c.requester_id) === String(currentUser?.id)));
      } catch {
      ...
      <div className="max-w-7xl mx-auto">
        ...
        {!loading && !error && filtered.length > 0 && (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {filtered.map(c => (
              <div key={c.id} className="bg-white -3xl shadow-xl border border-[#E5E7EB] hover:shadow-2xl transition-all duration-300 overflow-hidden hover:scale-[1.01]">
                ...
                <div className="p-6">
                  ...
                  {c.status === 'accepted' ? (
                    <button
                      ...
                      className="w-full flex items-center justify-center gap-2 px-4 py-3 bg-[#1A1F5E] text-white -full font-semibold transition-all hover:opacity-90 hover:scale-[1.02] shadow-lg"
                    >
                      <Activity className="w-5 h-5" /> Open Activities
                    </button>
                  ) : (
                    <button
                      onClick={() => navigate('/mentors')}
                      className="w-full flex items-center justify-center gap-2 px-4 py-3 border-2 border-[#1A1F5E] text-[#1A1F5E] -full font-semibold transition-all hover:bg-[#1A1F5E] hover:text-white"
                    >
        ...
        {!loading && !error && filtered.length === 0 && (
          <div className="text-center py-20">
            ...
            {!searchQuery && filterStatus === 'all' && (
              <button
                onClick={() => navigate('/mentors')}
                className="bg-[#1A1F5E] text-white px-8 py-3 -full font-semibold shadow-lg hover:opacity-90 hover:scale-105 transition-all"
              >
                Discover Mentors
              </button>
            )}
